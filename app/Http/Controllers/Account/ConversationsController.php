...
class ConversationsController extends AccountBaseController
{
	...
	public function messages($conversationId)
	{
		...
		$conversation = Message::where('id', $conversationId)->where(function($query) {
		...
		if ($conversation->is_read != 1) {
			if ($data['countMessages'] > 0) {
				// Check if the latest reply is from the current logged user
				if ($data['messages']->has(0)) {
					$latestReply = $data['messages']->get(0);
	...
	public function reply($conversationId, Request $request)
	{
		...
		$newMessage = [
			...
			'message'      => $request->input('message')
				. '<br><br>'
				. t('Related to the ad')
				. ': <a href="' . lurl($conversation->post->uri) . '">' . t('Click here to see') . '</a>',
		];
