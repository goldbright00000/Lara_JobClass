...
trait LocalizationTrait
{
	...
	public function getUrlThroughEnteredRoute($locale = null, $url = null, $attributes = [])
	{
		...
		// Search: Category
		if (
			str_contains($url, trans('routes.t-search-cat'))
			...
			str_contains($url, trans('routes.t-search-city'))
			...
			str_contains($url, trans('routes.page'))
			...
			str_contains($url, trans('routes.t-search'))
			&& !str_contains($url, trans('routes.t-search-cat'))
		) {
			// Get the Route Path
			$routePath = trans('routes.v-search', ['countryCode' => $countryCode], $locale);
