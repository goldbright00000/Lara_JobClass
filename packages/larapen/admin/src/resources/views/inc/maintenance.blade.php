	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">{{ __t('Maintenance Mode') }}</h4>
			</div>
			
			<form role="form" method="POST" action="{{ url(config('larapen.admin.route_prefix', 'admin') . '/maintenance/down') }}">
				{!! csrf_field() !!}
				...
				<div class="modal-footer">
					...
					<button type="submit" class="btn btn-primary">{{ __t('Put in Maintenance Mode') }}</button>
				</div>
			</form>
		</div>
		<!-- /.modal-content -->
